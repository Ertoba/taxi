...
class DriverNearByCubit extends Cubit<DriverNearByState> {
  ...
  Future<void> getNearbyDrivers({
    ...
    try {
      ...
      final center = GeoFirePoint(GeoPoint(pickupLat ?? 0.0, pickupLng ?? 0.0));
          ...
          .map((doc) {
            ...
            if (context != null) {
              ...
              if (lastActiveState != null &&
                  lastActiveState.toString() != "0" &&
                  lastActiveState.toString().isNotEmpty) {
                ...
                if (timestamp is Timestamp) {
                  ...
                  if (difference > 20) {
                    debugPrint(
                      "Driver ${doc.id} removed: last update $difference min ago",
                    );
                    return null;
                  }
                } else {
                  debugPrint(
                    "Driver ${doc.id} removed: no valid timestamp found",
                  );
                  return null;
                }
              } else {
                debugPrint(
                  "Driver ${doc.id} skipped: LastActiveApp is empty or 0",
                );
              }
            } else {
      ...
      if (navigatorKey.currentContext!
        ...
        final futures = driverChunks.map((chunk) async {
          final origins = chunk
              .map(
                (driver) => RouteCoordinate(
                  (driver['latitude'] as num).toDouble(),
                  (driver['longitude'] as num).toDouble(),
                ),
              )
              .toList();
          final matrix = await GoogleRoutesService.computeRouteMatrix(
            origins: origins,
          ...
          for (var i = 0; i < chunk.length; i++) {
            final route = matrix[i];
            if (route != null && route.distanceKm <= (distance ?? 15.0)) {
        ...
        emit(
          DriverUpdated(
            nearbyDrivers: finalFilteredDrivers,
...
class DriverMapCubit extends Cubit<DriverMapState> {
  ...
  Future<void> updateMapWithNearbyDrivers({
    ...
    try {
      ...
      final Uint8List markerIconPickUp = await getBytesFromAsset(
        pickupImage,
        15,
      );
