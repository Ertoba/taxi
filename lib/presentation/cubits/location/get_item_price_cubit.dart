...
class GetDistanceRouteCubit extends Cubit<GetDistanceRouteState> {
  ...
  Future<void> getDistanceAndFares({
    ...
    try {
      ...
      for (int i = 0; i < vehicleCategories.length; i++) {
        ...
        vehicleFares.add({
          "vehicleName": category.name?.trim() ?? "",
          "fare": fare.toString(),
          ...
          "serviceTypeId": context.read<ServiceTypeId>().state.value ?? '',
        });
  ...
  Future<Map<String, dynamic>> _fetchDistanceAndDuration({
    ...
    final routes = await GoogleRoutesService.computeRoutes(
      origin: RouteCoordinate(double.parse(pickupLat), double.parse(pickupLng)),
      destination: RouteCoordinate(
        double.parse(dropOffLat),
        double.parse(dropOffLng),
      ),
      computeAlternativeRoutes: true,
    );
    if (routes.isEmpty) return {};

    final bestRoute = routes.reduce(
      (current, candidate) =>
    ...
    return {
      ...
      'duration': GoogleRoutesService.formatDuration(bestRoute.durationSeconds),
    };
  }
