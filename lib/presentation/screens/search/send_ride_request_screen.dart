...
class _SendRideRequestScreenState extends State<SendRideRequestScreen> {
  ...
  Future<void> _fetchDistanceAndTime({
    ...
    if (useGoogleApi) {
      debugPrint('Fetching distance and ETA using Google Routes API v2');

      try {
        final routes = await GoogleRoutesService.computeRoutes(
          origin: RouteCoordinate(fromLat, fromLng),
          destination: RouteCoordinate(toLat, toLng),
        );
        if (routes.isNotEmpty) {
          final route = routes.first;
          final distanceText = GoogleRoutesService.formatDistance(
            route.distanceMeters,
          );
          final durationText = GoogleRoutesService.formatDuration(
          ...
          debugPrint(
            'Distance: $distanceText | Traffic-aware ETA: $durationText',
          );
        }
      } catch (e) {
        debugPrint('Routes API exception while fetching distance/time: $e');
      }
    } else {
